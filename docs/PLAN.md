...
### 2.4. Повторный поиск готовых решений (проверено 2026-09-09)

Целенаправленно искал репозитории «IG → мой TG-канал» и «ссылка → канал».
Ничего подходящего под задачу целиком не нашлось. Что есть:

**Заявляют IG → TG-канал:**
- `chdlc/instagram-to-telegram` — 22★, обновлён 12.08.2026. Ближайший аналог.
  instagrapi + логин/пароль от IG, опрос через GitHub Actions **раз в 6 часов**,
  чекпоинт опубликованного по SFTP. Фото/альбомы/видео есть.
  Режима «по ссылке» нет. Официального API нет.
- `subinps/InstaFeed` — 24★, помечен [WIP], 2 коммита, последняя активность
  12.2024. instaloader + файл сессии, нужен persistent-диск. Заброшен.

**Загрузчики по ссылке (отвечают в личку, не в канал):**
- `Mak5er/Downloader-Bot` — 60★, живой (08.09.2026), aiogram, много платформ,
  но middleware ограничивает бота приватными чатами; публикации в канал нет.
- `arashnm80/best-instagram-downloader` — 37★; `zerox9dev/Vidzilla` — 69★;
  `gth-ai/reclip-telegram-bot` — 53★; `sb0y/warp_beacon` — 21★.
  Все на скраперах/yt-dlp, все отдают медиа отправителю.

**Вывод:** ни одно решение не закрывает связку «официальный API + свой канал +
ссылка по требованию». Ближайшее (`chdlc`) требует пароль от Instagram,
опрашивает раз в 6 часов и не умеет режим по ссылке. Свой сервис на
официальном API получается и надёжнее, и меньше по объёму, чем допиливание
чужого скрапера.
