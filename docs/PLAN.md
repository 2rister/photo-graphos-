...
instagrapi / aiograpi  # ТОЛЬКО если нужны чужие посты
...
### Этап 7 (опционально)
- Fallback на `instagrapi` для чужих постов и Stories.
- Локальный Bot API server, если нужны видео > 50 МБ.
- Кнопка «Открыть в Instagram» под постом, фильтры по хэштегам,
  ручная модерация: сначала предпросмотр владельцу, публикация по кнопке.

## 5. Риски
| Риск | Митигация |
|---|---|
| Аккаунт не Professional | перевести в Creator/Business — иначе официального пути нет |
...
| Скрапинг (если понадобится) | нарушает ToS IG, риск бана; ломается раз в 2–4 недели |
